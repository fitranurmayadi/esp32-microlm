...
def run_serial_chat(port=None, baudrate=115200):
    ...
    if port is None:
        print("error: port serial (/dev/ttyUSB* atau /dev/ttyACM*) tidak ditemukan", file=sys.stderr)
        return

    print(f"kibo-client-id: terhubung ke {port} @ {baudrate} baud (Ctrl+C untuk keluar)\n")
    ...
    try:
        ...
        print(f"error: gagal membuka port {port}: {e}", file=sys.stderr)
        return
    ...
    try:
        ...
        print("\nkibo-client-id: koneksi ditutup")
    finally:
        running = False
        time.sleep(0.1)
        try:
